...
class CausalKnowledgeSystem:
    ...
    async def retrieve_causes(self, outcome: str, context: str) -> List[str]:
        ...
        """
        results = await self.knowledge_graph.execute_query(query, {"outcome": outcome, "context": context})
    ...
    async def retrieve_outcomes(self, action: str, context: str) -> List[str]:
        ...
        """
        results = await self.knowledge_graph.execute_query(query, {"action": action, "context": context})
        return [result["outcome"] for result in results]

    async def get_causal_chain(self, initial_action: str, depth: int = 3) -> List[Tuple[str, str]]:
        """
        ...
        """
        results = await self.knowledge_graph.execute_query(query, {"initial_action": initial_action})
