...
class ProceduralKnowledgeSystem:
    ...
    async def import_data(self, path: str):
        with open(path, 'r') as f:
            ...
            for tool_name, usages in self.tool_usage.items():
                for usage in usages:
                    await self.knowledge_graph.add_or_update_node("ToolUsage", {"tool_name": tool_name, "usage": usage})
    ...
    async def enhance_tool_usage(self, tool_name: str) -> str:
        ...
        Tool: {tool_name}
        """
        usage_insights = await self.llm.chat_with_ollama("You are a tool usage analysis expert.", prompt)
        ...
        for tool_name, usages in self.tool_usage.items():
            ...
            for usage in usages:
                ...
                for other_tool, other_usages in self.tool_usage.items():
                    if other_tool != tool_name:
                        other_embedding = self.embedding_manager.encode(other_usages[-1])
                        similarity = self.embedding_manager.cosine_similarity(embedding, other_embedding)
                        self.tool_graph.add_edge(tool_name, other_tool, weight=similarity)

    async def get_similar_tools(self, tool_name: str, k: int = 5) -> List[Dict[str, Any]]:
        ...
        return [{"tool": neighbor, "similarity": data['weight']} for neighbor, data in neighbors]
