...
class CommunityManager:
    ...
    async def build_graph_from_knowledge(self):
        ...
        relationships = await self.knowledge_graph.execute_query("MATCH (a)-[r]->(b) RETURN a.id, b.id, type(r)")
    ...
    async def summarize_communities(self):
        ...
        for community_id in set(self.communities.values()):
            community_nodes = [node for node in self.graph.nodes if self.communities[node] == community_id]
            community_content = " ".join([self.graph.nodes[node].get('content', '') for node in community_nodes])
        ...
        logger.info(f"Generated summaries for {len(self.community_summaries)} communities")
    ...
    async def query_communities(self, query: str) -> str:
        ...
        for community_id, summary in self.community_summaries.items():
            ...
            similarity = self.embedding_manager.cosine_similarity(query_embedding, summary_embedding)
        ...
        partial_answers = []
        for community_id, _ in relevant_communities[:3]:  # Consider top 3 most relevant communities
            community_nodes = [node for node in self.graph.nodes if self.communities[node] == community_id]
            community_content = " ".join([self.graph.nodes[node].get('content', '') for node in community_nodes])
            
            # Generate a partial answer based on the community content and the query
            partial_answer = await self.embedding_manager.generate_partial_answer(community_content, query)
            partial_answers.append(partial_answer)

        # Combine partial answers into a final answer
        final_answer = await self.embedding_manager.combine_partial_answers(partial_answers, query)
    ...
    async def update_knowledge(self, new_knowledge: Dict[str, Any]):
        ...
        await self.knowledge_graph.add_or_update_node(new_knowledge['label'], new_knowledge)
        ...
        logger.info(f"Updated knowledge and communities for new node: {new_knowledge['id']}")
